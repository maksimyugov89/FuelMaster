...
### Настройка переменных окружения (Dart / Firebase / API)

**Рекомендуемый способ (release и debug на устройстве):**

1. Скопируйте `env.defines.json.example` → `env.defines.json`.
2. Заполните реальными ключами (файл в `.gitignore`).
3. Запускайте приложение:
    ```bash
    flutter run --dart-define-from-file=env.defines.json
    ```
    Сборка release:
    ```bash
...
1. Скопируйте `android/local.properties.example` → `android/local.properties` (если файла ещё нет).
2. Укажите `yandex.maps.apikey` и при необходимости `yandex.mobile.ads.app.id`.

### Подпись release (Android)

1. Скопируйте `android/keystore.properties.example` → `android/keystore.properties`.
2. Укажите пути и пароли к вашему `*.jks` (файл keystore не коммитьте).
...
4.  **Настройка Yandex MapKit / Ads** — см. `android/local.properties.example`.
...
6.  **Запуск приложения**:
    ...
    ```bash
    flutter run -d RMX3853 --dart-define-from-file=env.defines.json --verbose
    ```
